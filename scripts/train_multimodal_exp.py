...
def create_model():
    if args.representation:
        ...
        model = ECCONetwork(radius_scale = 40,
                            layer_channels = [8, 16, 16, 16, 8], #[8, 24, 24, 24, 3], #[16, 32, 32, 32, 3], 
...
def train():
    ...
    def train_one_batch(model, batch, loss_f, train_window=2):
        ...
        sigma0 = sigma0 + calc_sigma_edit(pr_m1[:,:,:2,:])
        gt_pos1 = batch['pos1']

        losses = loss_f(pr_pos1, gt_pos1, sigma0, batch['car_mask'].squeeze(-1), prob = pr_m1[...,2,0])
        del gt_pos1
        pos0 = batch['pos0']
        vel0 = batch['vel0']
...
def evaluate(model, val_dataset, loss_f, use_lane=False,
    ...
    for i, sample in enumerate(val_dataset):
        ...
        losses = loss_f(pr_pos1, gt_pos1, sigma0, data['car_mask'].squeeze(-1), prob = pr_m1[...,2,0])
