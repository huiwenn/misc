...
parser.add_argument('--cuda_visible_devices', default='1,2,3,4')
...
parser.add_argument('--modes', default=2, type=int, help='number of modes')
...
def create_model():
    from models.rho_reg_ECCO_multi import PECCONetworkMulti
    """Returns an instance of the network for training and evaluation"""
    #output_ch = 4 * args.modes # loc, M, M, likelihood
    model = PECCONetworkMulti(radius_scale = 40,
...
def train():
    ...
    def train_one_batch(model, batch, loss_f, train_window=2):
        ...
        sigma0 =[calc_sigma_edit(m0) for _ in range(args.modes)]
        U = calc_u(sigma0[0])
        inputs = ([
        ...
        inputs_allmodes = [inputs for _ in range(args.modes)]
      
        pr_pos1, pr_vel1, pr_m1, states, p = model(inputs_allmodes)
    ...
    for i in range(epochs):
        ...
        for batch_itr in range(batches_per_epoch * args.batch_divide):
            ...
            if batch_itr == batches_per_epoch*args.batch_divide - 1:
        ...
        with torch.no_grad():
            ...
            #riter.add_scalar('MRS', result['mis'], num_samples)
...
def evaluate(model, val_dataset,loss_f, use_lane=False,
    ...
    for i, sample in enumerate(val_dataset):
        ...
        batch = process_batch(sample, device)
        batch_size = batch['pos_2s'].shape[0]
        ...
        lane = batch['lane']
        lane_normals = batch['lane_norm']
        agent_id = batch['agent_id']
        city = batch['city']
        scenes = batch['scene_idx']

        m0 = -5*torch.eye(2, device=device).reshape((1,2,2)).repeat((batch_size, 60, 1, 1))
        sigma0 = [calc_sigma_edit(m0) for _ in range(args.modes)]
        U = calc_u(calc_sigma_edit(m0))
        inputs = ([
            batch['pos_2s'], batch['vel_2s'],
            batch['pos0'], batch['vel0'],
            batch['accel'], U, 
        ...
        inputs_allmodes = [inputs for m in range(args.modes)]
        #print(inputs_allmodes)
        pr_pos1, pr_vel1, pr_m1, states, p = model(inputs_allmodes)
