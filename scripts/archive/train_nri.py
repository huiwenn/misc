...
import argparse
import datetime
from evaluate_nri import evaluate
...
parser.add_argument('--val_window', default=29, type=int, help='how many timestamps to iterate in validation')
...
def create_model():
    ...
    model = ParticlesNetwork(radius_scale = 1,
...
def train():
    #am = ArgoverseMap()
    log_dir = "runs/nri" + model_name + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    ...
    def train_one_batch(model, batch, loss_f, train_window=2):
        ...
        m0 = torch.zeros((batch['pos_enc'].shape[0], 5, 2, 2), device=device)
        inputs = ([
