...
def train():
    ...
    def train_one_batch(model, batch, loss_f, train_window=2):
        ...
        inputs = ([
            ...
            batch['accel'], torch.cat([U0, U0], dim=-2),
        ...
        for i in range(train_window-1):
            ...
            inputs = (pos_enc, vel_enc, pr_pos1, pr_vel1, batch['accel'],
                      torch.cat([U0, U1], dim=-2), 
                      batch['lane'], batch['lane_norm'], 
                      batch['car_mask'], batch['lane_mask'])

            pos0, vel0, U0 = pr_pos1, pr_vel1, U1
            # del pos_enc, vel_enc
...
def evaluate(model, val_dataset, loss_f, use_lane=False,
    ...
    for i, sample in enumerate(val_dataset):
        ...
        inputs = ([
            ...
            data['pos0'], data['vel0'], 
            data['accel'], torch.cat([U0, U0], dim=-2),
        ...
        sigma = pr_agent[:, -4:].reshape(pr_agent.shape[0],2,2)
        p = torch.distributions.MultivariateNormal(pr_agent[:, :2], sigma)
    ...
    for k, v in prediction_gt.items():
        ...
        mis[k] = mis_loss(v[0][:,:2], v[1],v[0][:,3:].reshape(train_window,2,2),sigma_ready=True)
