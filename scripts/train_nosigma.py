...
import argparse
import datetime
from evaluate_nosigma import evaluate
...
def train():
    ...
    def train_one_batch(model, batch, loss_f, train_window=2):
        ...
        inputs = ([
            ...
            batch['accel'], None, #other feats: 4x2 two M matrices
        ...
        for i in range(train_window-1):
            ...
            losses += loss_f(pr_pos1, gt_pos1, None, batch['car_mask'].squeeze(-1))
    ...
    for i in range(epochs):
        ...
        for batch_itr in range(batches_per_epoch * args.batch_divide):
            ...
            current_loss = train_one_batch(model, batch_tensor, ecco_loss, train_window=args.train_window)
        ...
        with torch.no_grad():
            ...
            valid_total_loss, _ = evaluate(model.module, val_dataset, ecco_loss, train_window=args.val_window,
