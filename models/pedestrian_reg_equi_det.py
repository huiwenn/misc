...
class ParticlesNetwork(nn.Module):
    def __init__(self, 
        ...
        self.conv_fluid = RelEquiCtsConv2dRho1ToReg(in_channels = self.in_channel, 
        ...
        for i in range(1, len(self.layer_channels)-1):
            ...
            denses.append(dense)
            conv = RelEquiCtsConv2dRegToReg(in_channels = in_ch, 
                                             out_channels = out_ch,
                                             num_radii = self.num_radii, 
                                             num_theta = self.num_theta,
                                             radius = self.radius_scale, 
        ...
        denses.append(dense)
        conv = RelEquiCtsConv2dRegToRho1(in_channels = in_ch, 
                                     out_channels = out_ch,
                                     num_radii = self.num_radii, 
                                     num_theta = self.num_theta,
                                     radius = self.radius_scale, 
    ...
    def forward(self, inputs, states=None):
        ...
        return p_corrected, v_corrected, (feats_v, feats_p)
