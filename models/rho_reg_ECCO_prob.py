...
class ECCONetwork(nn.Module):
    def __init__(self, 
        ...
        '''
        prob1 = nn.Sequential(
            EquiLinearRegToReg(in_ch, in_ch, self.reg_dim),
            EquiLinearRegToRho1(self.reg_dim), 
        )

        prob2 = nn.Sequential(
            EquiLinearRegToReg(in_ch, 2, self.reg_dim),
            EquiLinearRegToRho1(self.reg_dim), 
        )
        probs = [prob1, prob2]
        self.probs = nn.ModuleList(probs)
        '''
    ...
    def compute_correction(self, p, v, other_feats, box, box_feats, fluid_mask, box_mask):
        ...
        #self.pos_correction[...,0,:] = (1.0 / 32) * self.pos_correction[...,0,:]
